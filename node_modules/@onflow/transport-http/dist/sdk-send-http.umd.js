(function (global, factory) {
  ...
  const send$1 = function (addr, tag, data) {
  ...
  const spawn = function (fn) {
    ...
    const ctx = {
      ...
      send: function (to, tag, data) {
        let opts = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
        opts.from = addr;
        return send$1(to, tag, data, opts);
      },
      sendSelf: (tag, data, opts) => {
        if (root.FCL_REGISTRY[addr]) send$1(addr, tag, data, opts);
      },
      broadcast: function (tag, data) {
        let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
        opts.from = addr;
        for (let to of root.FCL_REGISTRY[addr].subs) send$1(to, tag, data, opts);
      },
      ...
      fatalError: error => {
        ...
        for (let to of root.FCL_REGISTRY[addr].subs) send$1(to, UPDATED$2);
      }
    };
   ...
   */
  function all$1() {
    return send$1(NAME$1, GET_ALL$1, null, {
      expectReply: true,
      timeout: 10
    });
  }
   ...
   */
  function all() {
    return send$1(NAME, GET_ALL, null, {
      expectReply: true,
      timeout: 10
    });
  }
...
See more here: https://docs.onflow.org/fcl/reference/sdk-guidelines/#connect`,
  ...
  async function sendExecuteScript(ix) {
    ...
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Execute Script Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Execute Script Error: context.response must be defined.`);
    invariant$2(context.Buffer, `SDK Send Execute Script Error: context.Buffer must be defined.`);
    ix = await ix;
    if (ix.block.id) {
  ...
  async function sendGetAccount(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Get Account Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Get Account Error: context.response must be defined.`);
    invariant$2(context.Buffer, `SDK Send Get Account Error: context.Buffer must be defined.`);
    ix = await ix;
  ...
  async function sendGetBlockHeader(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Get Block Header Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Get Block Header Error: context.response must be defined.`);
    ix = await ix;
  ...
  async function sendGetBlock(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Get Block Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Get Block Error: context.response must be defined.`);
    ix = await ix;
    const interactionHasBlockID = ix.block.id !== null;
    const interactionHasBlockHeight = ix.block.height !== null;
    if (interactionHasBlockID) {
  ...
  async function sendGetCollection(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Get Collection Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Get Collection Error: context.response must be defined.`);
    const httpRequest$1 = opts.httpRequest || httpRequest;
    const res = await httpRequest$1({
      hostname: opts.node,
  ...
  async function sendGetEvents(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Get Events Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Get Events Error: context.response must be defined.`);
    invariant$2(context.Buffer, `SDK Send Get Events Error: context.Buffer must be defined.`);
    ix = await ix;
    ...
    invariant$2(interactionContainsBlockHeightRange || interactionContainsBlockIDsList, "SendGetEventsError: Unable to determine which get events request to send. Either a block height range, or block IDs must be specified.");
  ...
  async function sendGetTransaction(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Get Transaction Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Get Transaction Error: context.response must be defined.`);
    invariant$2(context.Buffer, `SDK Send Get Transaction Error: context.Buffer must be defined.`);
    const httpRequest$1 = opts.httpRequest || httpRequest;
    ix = await ix;
    const res = await httpRequest$1({
      hostname: opts.node,
  ...
  async function sendGetTransactionStatus(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Get Transaction Status Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Get Transaction Status Error: context.response must be defined.`);
    invariant$2(context.Buffer, `SDK Send Get Transaction Status Error: context.Buffer must be defined.`);
    const httpRequest$1 = opts.httpRequest || httpRequest;
    ix = await ix;
    const res = await httpRequest$1({
      hostname: opts.node,
  ...
  async function sendPing(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Ping Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Ping Error: context.response must be defined.`);
    const httpRequest$1 = opts.httpRequest || httpRequest;
    await httpRequest$1({
      hostname: opts.node,
  ...
  async function sendTransaction(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Transaction Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Transaction Error: context.response must be defined.`);
    invariant$2(context.Buffer, `SDK Send Transaction Error: context.Buffer must be defined.`);
    const httpRequest$1 = opts.httpRequest || httpRequest;
    ix = await ix;
  ...
  async function sendGetNetworkParameters(ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Get Network Parameters Error: opts.node must be defined.`);
    invariant$2(context.response, `SDK Send Get Network Parameters Error: context.response must be defined.`);
    const httpRequest$1 = opts.httpRequest || httpRequest;
    ix = await ix;
    const res = await httpRequest$1({
      hostname: opts.node,
  ...
  const send = async function (ix) {
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    invariant$2(opts.node, `SDK Send Error: Either opts.node or "accessNode.api" in config must be defined.`);
    invariant$2(context.ix, `SDK Send Error: context.ix must be defined.`);
    ix = await ix;
